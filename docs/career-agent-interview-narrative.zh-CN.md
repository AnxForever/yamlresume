...
- §4 是十一个「问题 → 根因 → 修法 → 证据 → 追问预演」的故事，按被追问的概率排序。
...
### 4.11 语义检索：校准比模型重要，而且下游没有测出差异（2026-09-23）

**问题。** 词法匹配器对换了说法的证据是盲的：JD 说「provision Kubernetes clusters on bare
metal」，简历说「ran k8s on physical servers」，判 missing；中文简历对英文需求永远 missing。
在 20 组合成的改写金标集上，它的召回率是 0.056。主投岗位又点名 RAG。

**第一次根因。** 想当然地给 multilingual-e5-small 定了固定余弦阈值 0.85。扫描发现这个阈值
是**语言偏置**的：中文案例召回 1.0，英文 0.1。原因是 e5 的余弦分布压在一个很窄的高区间，
而且区间位置随语言漂移；"相关"与"无关"之间只差 0.03 到 0.10。

**修法。** 不再用绝对阈值。先算每个查询对 8 句中英文中性句子的平均相似度（它"跟什么都不像"
的基线），再要求证据高出基线至少 0.10。用同一金标集比较三种规则：绝对阈值、背景差值、
z 分数。只有 margin ≥ 0.10 满足预先写下的门槛（精确率 ≥ 0.8、召回 ≥ 词法 + 0.3、陷阱案例
零语义误报）：P 0.82 / R 0.50，两种语言的召回回到同一量级。0.08 的 F1 更高但精确率 0.76，
记录为备选而不是默认，因为这个项目里错的证据比漏的证据更贵。

**然后是诚实的部分。** 用同一裁判（词法 coverage + 金标关键词）跑 DeepSeek campaign，
5 案例 × 3 轮：词法 11/15，混合 11/15，覆盖率 0.602 对 0.609，区间完全重叠。**没有差异。**
原因写在 Feature Brief 里：这些案例的证据索引只有 13 到 19 条短文本，草拟 prompt 本来就
装得下全部证据，模型自己会把 k8s 改写成 Kubernetes。中文案例在混合模式下甚至从 2/3 掉到
0/3，样本太小不能定因，写成了可证伪假设和下一步实验。语义匹配因此**默认关闭**，只留开关。

**证据。** [语义证据检索](./resume-agent-semantic-evidence-retrieval.zh-CN.md) §7、§12；
`packages/resume-agent/src/retrieval/`；`scripts/retrieval-gold.ts` 的扫描表；
`scripts/campaign.ts` 的两份报告；提交 13479eb、b7b3490、ac3aed2。

**追问预演。**
- 为什么裁判不换成语义的？换了就是让新匹配器给自己打分，同一份简历会因为裁判更宽松而
  「变好」，before / after 不可比。裁判分离是这个切片最重要的决定。
- 那这个切片算失败吗？检索层的指标是真实提升（召回 0.06 → 0.50），下游没测出差异也是
  真实结论；两者都比"上了 RAG"这四个字有信息量。收益应该出现在两处没量到的地方：工作台
  的缺口展示，以及 prompt 装不下证据的大输入。
- 为什么不用 Provider 的 embeddings 接口？DeepSeek 没有（三个模型名探针全 404）；本地
  e5-small 130 MB、无密钥、可复现，代价是首次加载 15 秒和必须按模型重新校准。
- campaign 的 provenance 有个瑕疵：报告里的 runtimeRevision 是运行时 HEAD，而代码还没
  提交。已经写进文档；下次先提交再跑。
...
- 语义检索切片：已完成（§4.11）。下一步是在工作台展示语义命中与 passage 定位，并在
  大输入（prompt 装不下全部证据）上重新度量下游差异。
