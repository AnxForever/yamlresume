...
| RA-001B | Common resume/JD document ingestion | Partial implementation; trusted detection plus ODT/RTF/legacy DOC extraction enabled for development | Content signatures, fatal text decoding, bounded DOCX/ODT ZIP inspection, namespace-aware ODT extraction, bounded RTF scanner, real Apache POI Word 97 CFB fixture, legacy DOC unit/API multipart tests, stable API/Run errors and adversarial tests | Partial | Backfilled | Unicode/heterogeneous corpus, true worker isolation, RTF real-provider/Run-restart evidence, fuzz and operational limits |
...
| RA-013 | Career Agent Web workbench | Implemented for development; not operational | Capabilities-driven launcher, async Run polling/event projection, typed HITL controls, artifact preview/download, composer interaction polish, 190 frontend tests, TypeScript and production build | Partial | None | Real-browser/provider E2E, accessibility audit, large-artifact performance, SSE and file-answer upload loop |
