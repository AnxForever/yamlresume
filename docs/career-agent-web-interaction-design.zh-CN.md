...
当前启动器的空态保持克制：输入区外不放重复的引导标题或说明，输入框内使用
`请输入文字` 占位符；附件入口固定在左下角，发送入口固定在右下角。已有对话、文件标签和
错误只在实际发生后显示，避免默认状态被提示文案占满。
...
后端 HITL 设计定义 10 种类型，前端已实现 9 种可直接回答的控件，并对 `file` 做安全降级：

| control.type | 状态 | 渲染 | 校验来源 |
| --- | --- | --- | --- |
| `text` | 已实现 | 单行输入 | `minLength` / `maxLength` |
| `textarea` | 已实现 | 多行 + 字数 | 同上 |
| `single_choice` | 已实现 | 单选卡片 2–5 项 + 可选自定义 | 后端声明是否允许自定义 |
| `multi_choice` | 已实现 | 复选选项 + 数量校验 | 个数上下限 |
| `number` | 已实现 | 数字输入 | 数值范围 |
| `date` / `date_range` | 已实现（文本格式） | 文本输入并显示格式提示 | 后端最终校验 |
| `url` | 已实现 | 输入 + 协议提示 | 后端最终校验 |
| `file` | 安全降级 | 只读说明并允许重新发起 | 文件上传闭环仍缺失 |
| `confirm` | 已实现 | 两个显式按钮 | 后端最终校验 |
