...
| 数字 | 值 | 来源 |
| --- | ---: | --- |
| Agent 三包源码（不含许可头） | 25.9K 行 / 88 个文件 | `node scripts/project-metrics.mjs` |
| 测试 | 711 个用例 / 54 个文件 / 19.1K 行 | `pnpm test` 与同上脚本 |
| 设计与证据文档 | 30 篇 / 8.1K 行 | 同上脚本 |
...
| 语义证据检索（RA-018） | 改写金标集召回 0.06 → 0.50（精确率 0.82）；下游 campaign 5×3 两种匹配器均 11/15，无差异 | [语义证据检索 §12](docs/resume-agent-semantic-evidence-retrieval.zh-CN.md#12-校准与-campaign-结果2026-09-23) |
